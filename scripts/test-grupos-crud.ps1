...
Write-Host "📁 TEST: CRUD Completo de Grupos" -ForegroundColor Cyan
Write-Host "=" * 60
...
# 2. LISTAR GRUPOS
Write-Host "`n📋 Paso 2: Listar todos los grupos..." -ForegroundColor Yellow
try {
    $response = Invoke-RestMethod `
        -Uri "$baseUrl/api/v1/grupos" `
        -Method GET `
        -Headers $headers
    
    Write-Host "✅ Total de grupos: $($response.Count)" -ForegroundColor Green
    Write-Host "   Grupos:" -ForegroundColor Gray
    
    foreach ($grupo in $response | Select-Object -First 5) {
        Write-Host "   - [ID:$($grupo.id)] $($grupo.nombre) - $($grupo.descripcion)" -ForegroundColor Gray
    }
}
catch {
    Write-Host "❌ Error al listar grupos:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
}

# 3. CREAR NUEVO GRUPO
Write-Host "`n➕ Paso 3: Crear nuevo grupo..." -ForegroundColor Yellow
try {
    ...
    $grupoId = $created.id
    Write-Host "✅ Grupo creado exitosamente" -ForegroundColor Green
    ...
    Write-Host "❌ Error al crear grupo:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
...
Write-Host "`n🔎 Paso 4: Obtener grupo por ID..." -ForegroundColor Yellow
try {
    $grupo = Invoke-RestMethod `
        -Uri "$baseUrl/api/v1/grupos/$grupoId" `
        -Method GET `
        -Headers $headers
    
    Write-Host "✅ Grupo obtenido:" -ForegroundColor Green
    Write-Host "   ID: $($grupo.id)" -ForegroundColor Gray
    Write-Host "   Nombre: $($grupo.nombre)" -ForegroundColor Gray
    Write-Host "   Descripción: $($grupo.descripcion)" -ForegroundColor Gray
}
catch {
    Write-Host "❌ Error al obtener grupo:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
}

# 5. ACTUALIZAR GRUPO (PUT)
Write-Host "`n✏️ Paso 5: Actualizar grupo (PUT)..." -ForegroundColor Yellow
try {
    $updateGrupo = @{
        nombre = "$($grupo.nombre) - MODIFICADO"
    ...
    Write-Host "✅ Grupo actualizado exitosamente" -ForegroundColor Green
    Write-Host "   Nombre: $($updated.nombre)" -ForegroundColor Gray
    Write-Host "   Descripción: $($updated.descripcion)" -ForegroundColor Gray
}
catch {
    Write-Host "❌ Error al actualizar grupo:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
}

# 6. ELIMINAR GRUPO
Write-Host "`n🗑️ Paso 6: Eliminar grupo..." -ForegroundColor Yellow
try {
    ...
    Write-Host "✅ Grupo eliminado exitosamente (204 No Content)" -ForegroundColor Green
}
catch {
    Write-Host "❌ Error al eliminar grupo:" -ForegroundColor Red
    Write-Host $_.Exception.Message -ForegroundColor Red
}
...
try {
    ...
    Write-Host "⚠️  Grupo todavía existe" -ForegroundColor Yellow
}
catch {
    if ($_.Exception.Response.StatusCode.value__ -eq 404) {
        Write-Host "✅ Confirmado: Grupo eliminado (404)" -ForegroundColor Green
    }
    else {
...
Write-Host "   GET    /api/v1/grupos/{id}" -ForegroundColor Gray
Write-Host "   POST   /api/v1/grupos" -ForegroundColor Gray
Write-Host "   PUT    /api/v1/grupos/{id}" -ForegroundColor Gray
Write-Host "   DELETE /api/v1/grupos/{id}" -ForegroundColor Gray
